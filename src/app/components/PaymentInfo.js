...
export default function PaymentInfo({ activeTheme }) {
  ...
  const getStatusBadge = () => {
    ...
    const { details_submitted, charges_enabled, payouts_enabled, needsBankAccount, needsTosAcceptance, needsDocumentVerification } = accountStatus;
    ...
    if (needsDocumentVerification) issues.push('mangler KYC-dokumenter');
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setError(err.message || 'Kunne ikke åpne onboarding.');
                    } finally {
                      setConnecting(false);
                    }
                  }}
                  disabled={connecting}
                  className="w-full rounded-full border border-amber-300 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-700 transition hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-50"
                >
